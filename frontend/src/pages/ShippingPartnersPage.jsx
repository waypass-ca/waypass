...
export function ShippingPartnersPage({ onAddPartner, cases = [], onViewCase }) {
  ...
  async function handleConfirmAdd(enriched) {
    const created = await createShippingPartner({
      name: enriched.name,
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden bg-white">
      ...
      {addingPartner && (
        <AddPartnerModal crm={addingPartner} kind="shipping" onConfirm={handleConfirmAdd} onClose={() => setAddingPartner(null)} />
      )}
      ...
      <div className="border-b border-line bg-surface/80 backdrop-blur shrink-0 relative z-10">
        <div className="px-6 pt-6 pb-2 flex items-start justify-between gap-4">
          <PageTitle className="leading-none">Shipping Partners</PageTitle>
        </div>
