  ...
  return (
    <div className="bg-warm-white rounded-xl overflow-hidden border border-border mb-6">
      ...
      <div className="flex items-center justify-between px-6 py-4 border-b border-border">
        ...
        <button className="text-xs font-sans font-medium text-sage hover:text-sage/80 transition-colors cursor-pointer border-0 bg-transparent outline-none">
          View All →
        </button>
      </div>
      ...
      <table className="w-full">
        ...
        <tbody>
          {cases.map((c, i) => (
            <tr
              key={c.id}
              className={`border-t border-border hover:bg-cream/60 transition-colors ${i === cases.length - 1 ? '' : ''}`}
            >
