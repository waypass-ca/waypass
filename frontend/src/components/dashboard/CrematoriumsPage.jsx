...
function EditModal({ crm, onSave, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-charcoal/40 backdrop-blur-sm px-4">
      <div className="bg-warm-white rounded-2xl border border-border w-full max-w-lg shadow-xl overflow-hidden">
        <div className="px-6 py-5 border-b border-border flex items-center justify-between">
          <h2 className="font-display text-xl text-charcoal">Edit Partner</h2>
        ...
        <div className="px-6 py-5 space-y-4 max-h-[65vh] overflow-y-auto">
          {/* Status toggle */}
          <div className={`flex items-center justify-between p-3.5 rounded-xl border-2 transition-colors ${form.status === 'active' ? 'border-sage bg-sage-light/40' : 'border-border bg-cream'}`}>
            <div>
              <p className="font-sans text-sm font-medium text-charcoal">Active Partner</p>
            ...
            <button
              ...
              className={`w-11 h-6 rounded-full transition-all cursor-pointer flex-shrink-0 ml-6 relative border-0 outline-none ${form.status === 'active' ? 'bg-sage' : 'bg-border'}`}
            >
          ...
          {error && <p className="font-sans text-xs text-red-soft">{error}</p>}
        </div>
...
function DeleteModal({ crm, onConfirm, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-charcoal/40 backdrop-blur-sm px-4">
      <div className="bg-warm-white rounded-2xl border border-border w-full max-w-sm shadow-xl p-6">
        <div className="w-10 h-10 rounded-full bg-red-light flex items-center justify-center mb-4">
        ...
        <h2 className="font-display text-xl text-charcoal mb-1">Remove Partner</h2>
        <p className="font-sans text-sm text-muted leading-relaxed">
          Are you sure you want to remove <span className="text-charcoal font-medium">{crm.name}</span>? This cannot be undone.
        </p>
        {error && <p className="font-sans text-xs text-red-soft mt-3">{error}</p>}
        <div className="flex gap-2 mt-5">
          <Button variant="secondary" onClick={onClose}>Cancel</Button>
          <button
            onClick={handleDelete}
            disabled={deleting}
            className="flex-1 bg-red-soft text-white font-sans text-sm font-medium rounded-lg px-4 py-2 hover:opacity-90 transition-opacity disabled:opacity-50 cursor-pointer"
          >
    ...
    <div className="bg-warm-white rounded-xl border border-border overflow-hidden">
      <div className="px-5 py-4 flex items-start justify-between border-b border-border">
        ...
        <div className="flex items-center gap-2">
          <button
            onClick={() => onEdit(crm)}
            className="px-3 py-1.5 rounded-lg border border-border font-sans text-xs text-charcoal hover:bg-cream transition-colors"
          >
            Edit
          </button>
          <button
            onClick={() => onDelete(crm)}
            className="px-3 py-1.5 rounded-lg border border-border font-sans text-xs text-red-soft hover:bg-red-light transition-colors"
          >
            Delete
          </button>
        </div>
