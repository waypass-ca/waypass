...
export function LoginScreen() {
  ...
  if (signedUp) {
    return (
      <div className="flex items-center justify-center min-h-[calc(100vh-54px)] bg-cream">
        <div className="w-full max-w-sm text-center">
          <div className="w-12 h-12 rounded-full bg-sage-light flex items-center justify-center mx-auto mb-5">
            <svg className="w-6 h-6 text-sage" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" />
            </svg>
          </div>
          <h2 className="font-display text-2xl text-charcoal">Check your email</h2>
          <p className="font-sans text-sm text-muted mt-2 max-w-xs mx-auto">
            We sent a confirmation link to <span className="font-medium text-charcoal">{email}</span>. Click it to activate your account, then sign in.
          </p>
          <button
            onClick={() => { setSignedUp(false); switchMode('signin') }}
            className="mt-5 font-sans text-sm text-sage hover:text-sage/80 transition-colors cursor-pointer border-0 bg-transparent outline-none"
          >
            Back to sign in
          </button>
        </div>
  ...
  return (
    <div className="flex items-center justify-center min-h-[calc(100vh-54px)] bg-cream">
      <div className="w-full max-w-sm">
        ...
        <form onSubmit={handleSubmit} className="bg-warm-white rounded-xl border border-border p-8 space-y-4">
          <div>
          ...
          {error && (
            <p className="font-sans text-xs text-red-soft">{error}</p>
          )}
            ...
            {mode === 'signin' ? (
              <p className="font-sans text-xs text-muted">
                ...
                <button
                  ...
                  className="text-sage hover:text-sage/80 font-medium transition-colors cursor-pointer border-0 bg-transparent outline-none"
                >
                  Create one
                </button>
              </p>
