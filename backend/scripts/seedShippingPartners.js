 ...
 * Upserts everything into shipping_partners_db in Supabase.
 *
 * Searches for body-transport companies: funeral transport, mortuary transport,
 * body shipping, and remains transport services.
 *
 ...
 *
 * Usage: npm run seed:shippingPartners
 * Scope: SEED_PROVINCES=BC,QC npm run seed:shippingPartners
 */
...
// Multiple search terms to maximize coverage of body-transport companies
const SEARCH_QUERIES = [
  'funeral transport services',
  'body transport funeral',
  'mortuary transport',
  'remains transport services',
]
...
async function searchPlaces(query, lat, lng, radius, pageToken = null) {
  const url = pageToken
    ? `https://maps.googleapis.com/maps/api/place/textsearch/json?pagetoken=${encodeURIComponent(pageToken)}&key=${GOOGLE_KEY}`
    : `https://maps.googleapis.com/maps/api/place/textsearch/json?query=${encodeURIComponent(query)}&location=${lat},${lng}&radius=${radius}&key=${GOOGLE_KEY}`
...
async function upsertResults(results) {
  ...
  for (const place of results) {
    ...
    if (!error) upserted++
    else console.warn(`  Upsert error for ${place.name}: ${error.message}`)
  }
  return upserted
}
...
async function main() {
  ...
  console.log(`Seeding ${label} — ${provinces.length} province(s), ${tiles.length} tiles, ${SEARCH_QUERIES.length} queries each…`)
  console.log('To change scope: SEED_PROVINCES=BC,QC npm run seed:shippingPartners')
  ...
  for (const tile of tiles) {
    for (const query of SEARCH_QUERIES) {
      ...
      do {
        ...
        const data = await searchPlaces(query, tile.centerLat, tile.centerLng, tile.radius, pageToken)
        if (data.status !== 'OK' && data.status !== 'ZERO_RESULTS') {
          console.warn(`  Tile (${tile.centerLat.toFixed(1)},${tile.centerLng.toFixed(1)}) "${query}" page ${page}: ${data.status} — ${data.error_message ?? '(no error_message)'}`)
          break
        }
